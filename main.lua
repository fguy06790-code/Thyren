...
local uiName = "Thyren_Pro_Final_V14"
...
-- [[ CLEANUP: DELETE PREVIOUS INSTANCES ]]
...
-- =============================================================================
-- THYREN PRO ELITE (PART 2 OF 2) - 1:1 REPLICA WITH FADE & KEY FIX
-- =============================================================================
...
local ScreenGui = Instance.new("ScreenGui", CoreGui); ScreenGui.Name = "Thyren_Pro_Final_V14"
...
local Sidebar = Instance.new("Frame", Main); Sidebar.Size = UDim2.new(0, 160, 1, 0); Sidebar.BackgroundColor3 = Color3.fromRGB(55, 55, 55); Sidebar.BorderSizePixel = 0; Sidebar.Visible = false
...
local Container = Instance.new("Frame", Main); Container.Size = UDim2.new(1, -170, 1, -20); Container.Position = UDim2.new(0, 170, 0, 10); Container.BackgroundTransparency = 1; Container.Visible = false

local function CreateTabBtn(name, pos, page)
    local btn = Instance.new("TextButton", Sidebar); btn.Size = UDim2.new(0.85, 0, 0, 60); btn.Position = UDim2.new(0.075, 0, 0, pos); btn.BackgroundColor3 = Color3.fromRGB(40, 40, 40); btn.Text = name:upper(); btn.TextColor3 = Color3.fromRGB(110, 110, 110); btn.Font = Enum.Font.Michroma; btn.TextSize = 15; Instance.new("UICorner", btn).CornerRadius = UDim.new(0, 25)
...
CreateTabBtn("Macro", 130, MacroP); CreateTabBtn("Auto Parry", 210, ParryP)
...
local SwFrame = Instance.new("Frame", MacroP); SwFrame.Size = UDim2.new(0, 70, 0, 34); SwFrame.Position = UDim2.new(0.78, 0, 0, 165); SwFrame.BackgroundColor3 = Color3.fromRGB(155,155,155); Instance.new("UICorner", SwFrame).CornerRadius = UDim.new(1,0)
...
local Bind = Instance.new("TextButton", MacroP); Bind.Size = UDim2.new(1, 0, 0, 75); Bind.Position = UDim2.new(0, 0, 0, 215); Bind.BackgroundColor3 = Color3.fromRGB(50, 50, 50); Bind.Text = "CLICK TO BIND"; Bind.TextColor3 = Color3.fromRGB(105, 105, 105); Bind.Font = Enum.Font.Michroma; Bind.TextSize = 20; Instance.new("UICorner", Bind).CornerRadius = UDim.new(0, 35)

-- AUTO PARRY PAGE REPLICA
local POnOff = Instance.new("TextLabel", ParryP); POnOff.Size = UDim2.new(0.7, 0, 0, 30); POnOff.Position = UDim2.new(0,5,0,110); POnOff.Text = "AUTO PARRY ON/OFF"; POnOff.TextColor3 = Color3.fromRGB(0,0,0); POnOff.BackgroundTransparency = 1; POnOff.Font = Enum.Font.Michroma; POnOff.TextSize = 20; POnOff.TextXAlignment = Enum.TextXAlignment.Left
local PSwFrame = Instance.new("Frame", ParryP); PSwFrame.Size = UDim2.new(0, 75, 0, 38); PSwFrame.Position = UDim2.new(0.8, -5, 0, 110); PSwFrame.BackgroundColor3 = Color3.fromRGB(155,155,155); Instance.new("UICorner", PSwFrame).CornerRadius = UDim.new(1,0)
...
-- [[ KEY SYSTEM OVERLAY ]]
local Auth = Instance.new("Frame", Main); Auth.Size = UDim2.new(1, 0, 1, 0); Auth.BackgroundColor3 = Color3.fromRGB(75, 75, 75); Auth.BorderSizePixel = 0; Auth.ZIndex = 100
Instance.new("UICorner", Auth).CornerRadius = UDim.new(0, 24)
...
local function FadeIn(obj)
    ...
    for _, child in pairs(obj:GetDescendants()) do if child:IsA("TextLabel") or child:IsA("TextButton") or child:IsA("TextBox") then child.TextTransparency = 1 elseif child:IsA("Frame") then child.BackgroundTransparency = 1 end end
    TweenService:Create(obj, TweenInfo.new(0.8), {BackgroundTransparency = 0}):Play()
    for _, child in pairs(obj:GetDescendants()) do if child:IsA("TextLabel") or child:IsA("TextButton") or child:IsA("TextBox") then TweenService:Create(child, TweenInfo.new(0.8), {TextTransparency = 0}):Play() elseif child:IsA("Frame") then TweenService:Create(child, TweenInfo.new(0.8), {BackgroundTransparency = child:IsA("Frame") and child.BackgroundTransparency or 0}):Play() end end
end
...
Submit.MouseButton1Click:Connect(function()
    if IsKeyValid(KeyInput.Text) then
        Auth:Destroy()
        Sidebar.Visible = true; Container.Visible = true; Act.Visible = true; UpdateUI()
        FadeIn(Main); FadeIn(Act)
